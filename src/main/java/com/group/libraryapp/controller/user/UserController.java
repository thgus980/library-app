...
public class UserController { // API의 진입 지점으로써 HTTP Body를 객체로 변환하고 있다 (http, api 관련 작업)
    ...
    public UserController(UserService userService) { // UserController는 jdbcTemplate에 의존하고 있다(jdbTemplate가 없으면 동작하지 않는다), JdbcTemplate도 스프링 빈으로 등록되어 있다(build.gradle에서 dependencies 설정에 의해서 미리 만들어진 코드에 의해 스프링 빈으로 자동 등록되어 있음)
